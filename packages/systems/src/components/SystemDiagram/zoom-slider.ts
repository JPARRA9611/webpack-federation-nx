 ...
 * ```
 *
 * This is the basic HTML Structure that the Index creates as a sibling div of the diagram:
 * ```html
 * <div class="Index">
 ...
 *
 * If you want to experiment with this extension, try the <a href="../../extensionsJSM/Index.html">Zoom Slider</a> sample.
 * @category Extension
 */
export class ZoomSlider {
  ...
  private _opacity: number = 0.75;

  // Function used to keep the slider up to date
  private updateOnViewportBoundsChanged: (_e: go.DiagramEvent) => void;

  /**
   * Constructs a Index and sets up properties based on the options provided.
   * Also sets up change listeners on the Diagram so the Index stays up-to-date.
  ...
  constructor(diagram: go.Diagram, options?: { [index: string]: any }) {
    ...
    const self = this;
    this.updateOnViewportBoundsChanged = (_e: go.DiagramEvent) => {
      self.scaleToValue();
    };
    this.init();
  }
